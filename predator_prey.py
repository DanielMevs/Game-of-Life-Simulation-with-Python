...
class Human(Animal):
    def __init__(self, island, x=0,y=0,s="X"):
        ...
        self.breed_clock = self.breed_time
        self.hunt_cycle = self.hunt_ticks
    ...
    def clock_tick(self):
        ...
        if self.starve_clock <= 0:
            print("Human starved at {}{} ".format(self.x, self.y))
            self.island.remove(self)
    
    def hunt_eat(self):
        if not self.moved:
            loc_hunt = self.check_grid(Predator)
            loc_eat = self.check_grid(Prey)
            if loc_eat:
                self.island.remove(self.island.animal(loc_eat[0], loc_eat[1]))
                self.island.remove(self)
                self.x = loc_eat[0]
                self.y = loc_eat[1]
            ...
            if loc_hunt:
                self.island.remove(self.island.animal(loc_hunt[0], loc_hunt[1]))
                self.island.remove(self)
                self.x = loc_hunt[0]
                self.y = loc_hunt[1]
                self.island.register(self)
...
###########################################
def main(human_breed_time = 8, human_starve_time = 3, initialize_humans = 6, human_hunt_time = 10, predator_breed_time=6, predator_starve_time=3, initial_predators=10, prey_breed_time=3, initial_prey=50, \
         size=10, ticks=300):
    ...
    Human.hunt_ticks = human_hunt_time
    ...
    # make an island
    isle = Island(size,initial_prey, initial_predators, initialize_humans)
    print(isle)
    ...
    pylab.plot(time_list, human_list, label = "H")
    pylab.plot(time_list, predator_list, label = "X")
    pylab.plot(time_list, prey_list, label = "O")
